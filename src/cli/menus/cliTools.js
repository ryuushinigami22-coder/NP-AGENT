...
async function buildCodexHeader() {
  ...
  const { installed, hasNProuter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasNProuter, settings } = result.data;
  ...
  // Extract NProuter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:NProuter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract NProuter provider config
  const provider = settings?.models?.providers?.["nprouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("nprouter/") ? primary.replace("nprouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasNProuter, opencode } = result.data;
...
async function buildNPAgentHeader() {
  const result = await api.getCliToolSettings("npagent");
  if (!result.success) return `  ${COLORS.red}Failed to load settings${COLORS.reset}`;

  const { installed, hasNProuter, settings } = result.data;
  if (!installed) return `Status:   ${COLORS.red}✗ NPAgent Agent not installed${COLORS.reset}`;
...
async function npagentQuickSetup(port) {
  const { endpoint } = await getEndpoint(port);
  const apiKey = await getFirstApiKey();
  ...
  const model = await selectModelFromList("Select NPAgent Model", "", { excludeCombos: true });
  if (!model) return;

  const result = await api.applyCliToolSettings("npagent", { baseUrl: endpoint, apiKey, model });
  showStatus(result.success ? "NPAgent setup completed!" : `Failed: ${result.error}`, result.success ? "success" : "error");
  await pause();
}

async function npagentReset() {
  const result = await api.resetCliToolSettings("npagent");
  showStatus(result.success ? "NPAgent settings reset!" : `Failed: ${result.error}`, result.success ? "success" : "error");
  await pause();
}

async function showNPAgentMenu(port, breadcrumb = []) {
  await showMenuWithBack({
    title: "⚡ NPAgent Agent Settings",
    breadcrumb,
    headerContent: buildNPAgentHeader,
    refresh: async () => ({}),
    items: [
      { label: "⚡ Quick Setup", action: async () => { await npagentQuickSetup(port); return true; } },
      { label: "Reset to Default", action: async () => { await npagentReset(); return true; } }
    ]
  });
}
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use NProuter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
      ...
      {
        label: "NPAgent",
        action: async () => { await showNPAgentMenu(port, [...breadcrumb, "NPAgent"]); return true; }
      }
    ]
  });
}
