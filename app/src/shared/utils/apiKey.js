 ...
 * Format: skNP-{machineId}-{keyId}-{crc8}
...
export function generateApiKeyWithMachine(machineId) {
  ...
  const key = `skNP-${machineId}-${keyId}-${crc}`;
  return { key, keyId };
}
 ...
 * - New: skNP-{machineId}-{keyId}-{crc8}
 * - Old: sk-{random8}
 * @param {string} apiKey
 * @returns {{ machineId: string, keyId: string, isNewFormat: boolean } | null}
 */
export function parseApiKey(apiKey) {
  if (!apiKey || !apiKey.startsWith("skNP-")) return null;

  const parts = apiKey.split("-");
  
  // New format: skNP-{machineId}-{keyId}-{crc8} = 4 parts
  if (parts.length === 4) {
